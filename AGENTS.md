...
唯一入口：`src/entries/app.ts`；当前 **3 个域 / 14 页**：工艺（工艺流程图 / 符号库 / 工艺试算 / 工况预案）、
运行（运行数据 / 实时监视 / 泵站监视 / 能耗分项 / 精确曝气 / 加药优化）、运营（事件中心 / 污泥产运 / 设备资产 / 巡检管理）。
...
2. 撤下时机：**等"接下来会露出来的那一层"真正画完一帧**——`hideBootOverlayWhenPainted(ice)`
   ...
   **不要用"第 N 帧之后"这种固定时机**：启动段里 `recompute()`（建 14 个页签内容）会占住主线程几百毫秒，
...
- **入口是 `src/entries/boot.ts`**：只 `import './login-boot'`（挂登录门 + 撤启动遮罩），
  控制台（`src/entries/app.ts`：外壳 + 14 个页签 + 设计器 + 图表 + 案例数据）用 `import()` **异步**加载。
