...
export const SYMBOL_CATALOG: Record<WaterSymbolKind, SymbolEntry> = {
  ...
  storageTank: {
    ...
    tag: 'EQ',
    mediums: ['sewage'],
    role: '均衡水量水质、事故时暂存超标水（不使其进入后续工艺）',
    designFocus: ['调节容积（按 6~8 h 平均流量）', '事故池容积与回流方式', '防止沉积的搅拌或曝气'],
    checks: ['液位与溢流堰', '池底积泥与搅拌运行', '事故水回收的排放节奏'],
  },
  deodorizer: {
    kind: 'deodorizer',
    label: '除臭装置',
    ...
    tag: 'OD',
    mediums: ['air'],
    role: '收集预处理与污泥区的臭气，经生物滤池 / 化学洗涤后达标排放',
    designFocus: ['处理风量 m³/h', '换气次数', '填料层高度与停留时间'],
    checks: ['风机与风阀开度', '填料湿度与压差', '厂界臭气浓度'],
  },
  ...
  sludgeSilo: {
    ...
    tag: 'SIL',
    mediums: ['sludge'],
    role: '暂存脱水后泥饼，缓冲外运节奏（避免脱水机因等车停机）',
    designFocus: ['有效容积（按 1~2 d 泥量）', '卸料口与破拱方式', '料位计与满料联锁'],
    checks: ['料位是否接近满仓', '破拱 / 振动装置', '卸料口渗滤液收集'],
  },
  sludgeOut: {
    kind: 'sludgeOut',
    label: '污泥外运',
  ...
  submersiblePump: {
    kind: 'submersiblePump',
    label: '潜污泵',
    category: 'equipment',
    tag: 'P-SB',
    mediums: ['sewage', 'returnSludge', 'sludge'],
    role: '整机沉入池底或集水井的污水泵，回流污泥与外排污泥的主力',
    designFocus: ['流量与扬程', '通过粒径（防缠绕）', '自动耦合与备用台数'],
    checks: ['电流与振动', '集水井液位的启停联锁', '导杆耦合是否到位'],
  },
  screwPump: {
    kind: 'screwPump',
    label: '螺杆泵',
    category: 'equipment',
    tag: 'P-SC',
    mediums: ['sludge', 'chemical'],
    role: '输送高含固率污泥与药剂，流量稳定、剪切小（不破絮体）',
    designFocus: ['流量与压力', '转子 - 定子配合', '变频调速范围'],
    checks: ['出口压力与脉动', '定子磨损与泄漏', '干运行保护'],
  },
  vfd: {
    kind: 'vfd',
    label: '变频器',
    ...
    tag: 'VFD',
    mediums: ['air'],
    role: '泵与风机的调速装置：按需供气 / 供水，是吨水电耗的主要抓手',
    designFocus: ['功率与调速范围', '最低频率（防喘振 / 防沉泥）', '谐波治理'],
    checks: ['输出频率与实际工况是否匹配', '散热与滤网', '频率下限是否被误设'],
  },
  motorValve: {
    kind: 'motorValve',
    label: '电动阀',
    category: 'equipment',
    tag: 'MOV',
    mediums: ['sewage', 'effluent', 'recycle', 'sludge'],
    role: '可远程 / 联锁操作的阀门（超越管、联络管、回流管上的主力）',
    designFocus: ['通径与开闭时间', '联锁逻辑（与泵 / 液位配合）', '失电位置'],
    checks: ['开度反馈与限位', '手动 / 电动切换', '就地与中控的一致性'],
  },
  checkValve: {
    kind: 'checkValve',
    label: '止回阀',
    category: 'equipment',
    tag: 'CV',
    mediums: ['sewage', 'effluent', 'sludge'],
    role: '只允许单向流动，防止停泵时倒流冲击泵体与清水池',
    designFocus: ['通径与阻力损失', '关闭速度（防水锤）', '安装方向'],
    checks: ['关闭是否严密（有无倒流）', '水锤声与振动', '检修时的密封面'],
  },
  levelGauge: {
    kind: 'levelGauge',
    label: '液位计',
    ...
    tag: 'LT',
    mediums: ['sewage'],
    role: '池体 / 集水井液位，泵组启停与溢流预警的依据',
    designFocus: ['量程与盲区', '与泵组启停液位的配合', '防泡沫 / 防挂料'],
    checks: ['液位与人工尺量一致性', '探头清洁', '高液位与溢流报警是否有效'],
  },
  pressureGauge: {
    kind: 'pressureGauge',
    label: '压力表',
    ...
    tag: 'PT',
    mediums: ['sewage', 'air'],
    role: '泵出口与供气干管的压力监视，判断堵塞 / 堵塞前兆',
    designFocus: ['量程（工作压力 1.5~2 倍）', '取压点位置', '缓冲与隔离'],
    checks: ['与额定工况的偏差', '表针抖动（水锤）', '定期校验标签'],
  },
