...
画布控件那半（`ice-web-components` 的控件构造期取色）、以及**引擎**那半（画布底色 / 选中框 /
手柄 / 对齐引导线 / 阴影；图表 `theme:'auto'` 与设计器外壳都读它）。

入口在侧栏「**界面主题** → 浅色 / 深色」（子项文案带 ✓ 标出当前那套）。
**切换是就地热换、不刷新页面**（库 1.15.x 起：组件样式走**主题引用**，paint 时解析；
`setTheme()` 广播到所有注册过的引擎实例）。切完只有两件收尾：地址栏同步 `?theme=`
（`replaceState`，不产生历史记录）+ 菜单里的 ✓ 挪位（`ICEMenu.setItemLabel()`）。
口径与踩过的坑见 `AGENTS.md` 的「外观主题」一节，真机回归见 `e2e/theme.spec.ts`（6 条，
其中一条专门断言"切主题**没有**发生导航"）。

| 登录门（深色） | 工艺流程图（深色） |
|---|---|
| ![深色登录门](screenshots/20-dark-login.png) | ![深色工艺流程图](screenshots/21-dark-process.png) |
